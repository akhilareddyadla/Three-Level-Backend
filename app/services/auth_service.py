...
def save_facial_data(image_data: ImageModel):
    try:
        user_object_id = ObjectId(
            image_data.user_id.strip()
        )  # Ensure no extra spaces in user_id
        user = users_collection.find_one({"_id": user_object_id})
        ...
        )  # Strip off 'data:image/jpeg;base64,' part

        # Prepare the image entry for storage
        image_entry = {"user_id": image_data.user_id, "facial_image": image_bytes}

        # Save the facial image into the database
        result = users_collection.update_one(
            {"_id": user_object_id}, {"$set": {"facial_image": image_entry}}
        )

        if result.modified_count == 1:
            return {
                "message": "Facial image stored successfully",
        ...
        raise HTTPException(
            status_code=500, detail=f"An error occurred while saving facial data: {e}"
        )
...
def verify_facial_data(image_data: ImageModel):
    try:
        ...
        if not stored_image:
            raise HTTPException(
                status_code=404, detail="No facial image stored for this user"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"An error occurred during facial verification: {e}"
        )
